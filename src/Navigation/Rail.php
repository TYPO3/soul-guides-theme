 ...
 *
 * A slice of the same menu the bar gets, so the two cannot disagree about
 * where the reader is. Which section that is comes from the entry that says
 * `current` or `here`, and both come from one place, `Menu`.
 *
 * A section that is a single page has no rail at all. The bar's name for it
 * is the whole of what there is to say. Neither has the root, which is in no
 * section — the whole site is what the bar's menu carries, on every page
 * alike.
 */
final class Rail
{
