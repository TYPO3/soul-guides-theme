 ...
 * that renders one of this system's components and answers for half of it has
 * the worse of a theme's two failures. An author who reads the card
 * in Storybook and writes the page that shows it has to find out from the
 * render which half arrived. The part that did not goes by hand into their own
 * stylesheet, which is the thing the whole system exists to prevent. So `tag`,
 * `src` and `alt` are here because `sds-card` draws them, and anything it
 * gains lands here in the same commit.
 *
 * **And each has the spelling the element gives it.** `href` and `src` are the
 * names of everything in this system that links or takes a file. So an author
 * who has read the card in Storybook can write the directive with no lookup.
 * The two sides cannot drift into two vocabularies. A name of the theme's
    ...
    protected function processSub(
        ...
        return (new CardNode($title, $collectionNode->getChildren()))->withOptions([
            ...
            /* An author who wrote `:class:` meant it for their own stylesheet.
               ...
               must not do. Carried the way `topic` carries it. */
